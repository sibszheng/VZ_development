...
FeaturePlot(sce_10X_integrated[,sce_10X_integrated$clusters4=="6"], reduction = "tsne.harmony", features = "Cspg4",pt.size = 0.6)& 
...
sce_10X_all_NSC <- ScaleData(sce_10X_all_NSC,vars.to.regress="nCount_RNA", features=top20_NSC_cluster_markers)
...
sce_10X_all_EPC <- ScaleData(sce_10X_all_EPC,vars.to.regress="nCount_RNA", features=top50_EPC_cluster_markers)
DoHeatmap(sce_10X_EPC, features = top50_EPC_cluster_markers)

#Violin plot(Fig.S1B)
library(Seurat)
library(ggplot2)
sce_combined <- merge(sce_10X_integrated,y=sce_Plate)
...
library(ggplot2)
library(stringr)
NSC_early_markers_BP <- NSC_early_markers_BP[1:12,]
